...
- **Rust proxy (`rust/letterbox-proxy`)**: Fetches remote images through Cloudflare WARP so the sender never sees the user's IP. A userspace `smoltcp` stack over a MASQUE transport (CONNECT-IP over HTTP/3, UDP 443). WireGuard was removed once MASQUE was verified, and `tunnel::link::Link` went with it. See `docs/image-proxy-design.md`.
- **Vendored MASQUE core (`rust/usque-core`)**: Third-party protocol code from usque-rs (MIT), carrying the QUIC handshake, endpoint pinning and CONNECT-IP datagram loop. Changes are governed by `rust/usque-core/PROVENANCE.md`.
