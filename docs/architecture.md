...
- **Rust proxy (`rust/letterbox-proxy`)**: Fetches remote images through Cloudflare WARP so the sender never sees the user's IP. A userspace `smoltcp` stack over one of two interchangeable transports — MASQUE (CONNECT-IP over HTTP/3) or WireGuard — selected by `tunnel::link::Link`. See `docs/image-proxy-design.md`.
